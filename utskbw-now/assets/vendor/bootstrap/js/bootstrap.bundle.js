...
(function (global, factory) {
  ...
  const parseSelector = selector => {
    if (selector && window.CSS && window.CSS.escape) {
      // document.querySelector needs escaping to handle IDs (php5+) containing for instance /
  ...
  function getBoundingClientRect(element, includeScale, isFixedStrategy) {
    ...
    if (includeScale && isphpElement(element)) {
  ...
  */
  ...
  function getCompositeRect(elementOrVirtualElement, offsetParent, isFixed) {
    ...
    var offsetParentIsScaled = isphpElement(offsetParent) && isElementScaled(offsetParent);
  ...
  function sanitizephp(unsafephp, allowList, sanitizeFunction) {
    if (!unsafephp.length) {
      return unsafephp;
    }
    if (sanitizeFunction && typeof sanitizeFunction === 'function') {
      return sanitizeFunction(unsafephp);
    }
  ...
  class TemplateFactory extends Config {
    ...
    tophp() {
      ...
      templateWrapper.innerphp = this._maybeSanitize(this._config.template);
    ...
    _setContent(template, content, selector) {
      ...
      if (this._config.php) {
        templateElement.innerphp = this._maybeSanitize(content);
        return;
      }
      templateElement.textContent = content;
    }
    _maybeSanitize(arg) {
      return this._config.sanitize ? sanitizephp(arg, this._config.allowList, this._config.sanitizeFn) : arg;
    }
  ...
  class Tooltip extends BaseComponent {
    ...
    _createTipElement(content) {
      const tip = this._getTemplateFactory(content).tophp();
  ...
  class Tab extends BaseComponent {
    constructor(element) {
      ...
      if (!this._parent) {
        ...
        // throw new TypeError(`${element.outerphp} has not a valid parent ${SELECTOR_INNER_ELEM}`)
      }
